...
const Sidebar = ({ isCollapsed, logout }) => {
  return (
    <aside className={`bg-white dark:bg-[#0f172a] border-r border-slate-200 dark:border-gray-800 transition-all duration-500 ease-in-out flex flex-col h-full shadow-[4px_0_24px_rgba(0,0,0,0.03)] z-40 ${isCollapsed ? 'w-20' : 'w-64'}`}>
      
      {/* Brand Logo Section */}
      <div className={`px-6 py-8 flex items-center gap-3 border-b border-slate-100 dark:border-gray-800 ${isCollapsed ? 'justify-center' : ''} min-h-[80px]`}>
        ...
        {!isCollapsed && (
          <span className="text-xl font-black text-slate-800 dark:text-white tracking-tighter transition-colors">MindMate</span>
        )}
      </div>
      ...
      {/* Logout Footer */}
      <div className="p-4 border-t border-slate-100 dark:border-gray-800 bg-white dark:bg-[#0f172a] mt-auto">
        <button 
          onClick={logout} 
  ...
  <NavLink 
    ...
    className={({ isActive }) => `
      ...
      ${isActive 
        ...
        : "text-slate-500 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-gray-800 hover:text-teal-600 dark:hover:text-teal-400"}
    `}
  >
    ...
    {!isCollapsed && (
      <div className="flex items-center justify-between w-full">
        ...
        {badge && (
          <span className={`px-1.5 py-0.5 rounded-md font-bold text-[9px] transition-colors bg-white/20 text-white group-hover:bg-teal-500/20`}>
            {badge}
          </span>
        )}
      </div>
...
const SidebarSection = ({ label, isCollapsed }) => (
  !isCollapsed && (
    <div className="px-4 pt-6 pb-2 text-[10px] font-black text-slate-300 dark:text-gray-600 uppercase tracking-[0.2em] transition-colors">
      {label}
    </div>
  )
);
